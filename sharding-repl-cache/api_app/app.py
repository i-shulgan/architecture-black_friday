...
async def root():
    ...
    for collection_name in collection_names:
        ...
        try:
            async for stats in collection.aggregate([{"$collStats": {"count": {}}}]):
                shard_name = stats.get("shard", "unsharded")
                shard_document_counts[shard_name] = stats.get("count", 0)
    ...
    if topology_type == "Sharded":
        ...
        for shard in shards_list.get("shards", {}):
            ...
            members = shard["host"].split("/", maxsplit=1)[-1].split(",")
            shard_replica_counts[shard["_id"]] = len(members)
