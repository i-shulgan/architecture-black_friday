...
for service in configSrv shard1-1 shard2-1; do
...
try {
  ...
  rs.initiate({_id: "shard1Rs", members: [{_id: 0, host: "shard1-1:27017"}]});
}
EOF
...
try {
  ...
  rs.initiate({_id: "shard2Rs", members: [{_id: 0, host: "shard2-1:27017"}]});
}
EOF
...
if (!shardIds.includes("shard1Rs")) sh.addShard("shard1Rs/shard1-1:27017");
if (!shardIds.includes("shard2Rs")) sh.addShard("shard2Rs/shard2-1:27017");

sh.enableSharding("somedb");
const collection = db.getSiblingDB("somedb").helloDoc;
if (!collection.getShardDistribution) {
  throw new Error("MongoDB shell does not support shard distribution");
}
